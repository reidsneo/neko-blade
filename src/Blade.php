...
class Blade
{
    ...
    {
        global $app;
        $plugin_paths = array_map(function($val) use ($app) { return app_path()."/plugins/".str_replace(".","/",$val);} , $app->plugin_active);
        $view_paths = [app_path()."/themes/".$app->config->get('user_theme'),app_path()."/themes/a_".$app->config->get('admin_theme')];
        $view_paths = array_merge($plugin_paths,$view_paths);
        $view_cache_path = app_path()."/themes/_cache";
        $this->setFileExtension('.blade.php');
        $this->setViewFolder($view_paths);
        $this->setCacheFolder($view_cache_path);
    ...
    protected function compileStatements($statement)
    {
        ...
        return preg_replace_callback($pattern, function ($match) {
            ...
            if (isset(self::$directives[$match[1]])) {
                ...
                $count = null;
                if(is_array($match[3]) && is_object($match[3])){
                    $count = count($match[3]);
                }else if(is_string($match[3])){
                    $count = strlen($match[3]);
                }else{
                    throw new RuntimeException('Unknown method matching');
                }
    ...
    protected function findInPaths($name, $paths)
    {
        ...
        foreach ((array) $paths as $path) {
            $path = str_replace("/assets","",$path); //2/28/2021, 20:47:13 
            foreach ($this->getPossibleViewFiles($name) as $file) {
                $url_adm = $app->request->segment(1);
                if ($this->files->exists($viewPath = $path.'/'.$file)) {
                    return $viewPath;
                }else if(Str::contains($app->request->segment(1),$app->config['admin.url']) && Str::contains($path,$app->config['admin_theme']) ){
                    return $this->loadTemplateFile($name,$file,$path);
                }else if(!Str::contains($app->request->segment(1),$app->config['admin.url']) && Str::contains($path,$app->config['user_theme']) || $url_adm ==""){
                    return $this->loadTemplateFile($name,$file,$path);
                }
            }
        }
    ...
    protected function prepare($view)
    {
        ...
        foreach ($this->viewFolder as $path) {
            $path_file = $path."/".$view.$this->fileExtension;
            if(Str::contains($path_file, 'theme_admin'))
            {
                $admin_theme_path = app()->path."/app/themes/a_".app()->config['admin_theme']."/".str_replace("theme_admin/","",$view).$this->fileExtension;
                if(File::isExist($admin_theme_path))
                {
                    $actual = $admin_theme_path;
                    break;
                }
            }else if(Str::contains($path_file, 'theme_user'))
            {
                $user_theme_path = app()->path."/app/themes/".app()->config['user_theme']."/".str_replace("theme_user/","",$view).$this->fileExtension;
                if(File::isExist($user_theme_path))
                {
                    $actual = $user_theme_path;
                    break;
                }
            }else if(Str::contains($path_file, 'widgets'))
            {
                $path_widgets = app()->path."/app/plugins/".$view.$this->fileExtension;
                if(File::isExist($path_widgets))
                {
                    $actual = $path_widgets;
                    break;
                }
            }else{
                if(File::isExist($path_file))
                {
                    $actual = $path_file;
                    break;
                }
